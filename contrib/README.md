...
A Linux bash script that will set up traffic control (tc) to limit the outgoing bandwidth for connections to the Bitcoin Monster network. This means one can have an always-on bitcoinmonsterd instance running, and another local bitcoinmonsterd/bitcoinmonster-qt instance which connects to this node and receives blocks from it.
