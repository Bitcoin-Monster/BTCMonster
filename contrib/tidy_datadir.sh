...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Coin2Fly database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Coin2Fly datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Coin2Fly datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Coin2Fly 0.7 datadir."
    ;;
  3)
    echo "Detected Coin2Fly pre-0.8 datadir."
    ;;
  4)
    echo "Detected Coin2Fly 0.8 datadir."
    ;;
esac
