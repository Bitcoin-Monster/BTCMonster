...
*Setup instructions for a Gitian build of BitcoinMonster Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the BitcoinMonster
...
- [Building BitcoinMonster Core](#building-bitcoinmonster-core)
...
Clone the git repositories for BitcoinMonster Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/banqcoin/bitcoinmonster
```
...
in the BitcoinMonster Core repository under 'Fetch and build inputs' to install sources which require
...
To build BitcoinMonster Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the BitcoinMonster Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bitcoinmonster/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/banqcoin/bitcoinmonster
...
For example:
```bash
URL=https://github.com/crowning-/bitcoinmonster.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit bitcoinmonster=${COMMIT} --url bitcoinmonster=${URL} ../bitcoinmonster/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bitcoinmonster=${COMMIT} --url bitcoinmonster=${URL} ../bitcoinmonster/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bitcoinmonster=${COMMIT} --url bitcoinmonster=${URL} ../bitcoinmonster/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the bitcoinmonster git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../bitcoinmonster/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/banqcoin/bitcoinmonster-detached-sigs.git

BTCPATH=/some/root/path/bitcoinmonster.git
SIGPATH=/some/root/path/bitcoinmonster-detached-sigs.git

./bin/gbuild --url bitcoinmonster=${BTCPATH},signature=${SIGPATH} ../bitcoinmonster/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bitcoinmonster-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bitcoinmonster-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/bitcoinmonster-osx-build.assert
```
...
[bitcoinmonster/gitian.sigs](https://github.com/banqcoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
